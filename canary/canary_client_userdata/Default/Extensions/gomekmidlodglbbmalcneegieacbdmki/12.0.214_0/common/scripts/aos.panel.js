...
(function (Ractive) {
  ...
  Avast.ABEK = {
    Panel: {
      ...
      initMessaging: function(msgsId, tabId, url) {
        ...
        port.postMessage({type: 'init', tabId: tabId, url: url});
        return {
  ...
  var decorators = {
    toggleClassOnClick : function (node, classId, contentLen) {
      var eventName = 'click';
      var clickToggle = function() {
        if (contentLen && contentLen > 0) {
          var nClass = node.className;
          if (new RegExp("(?:^|\\s+)" + classId + "(?:\\s+|$)").test(nClass)) {
            // remove
            node.className = nClass.replace(new RegExp("(?:^|\\s+)" + classId + "(?:\\s+|$)", "g"), "");
          } else {
            this.className = nClass ? [nClass, classId].join(' ') : classId;
          }
        }
      };

      node.addEventListener(eventName, clickToggle, false);

      return {
        teardown: function () {
          node.removeEventListener(eventName, clickToggle, false);
        }
      };
    }
  };
  ...
  Avast.BE = {
    Panel: {
      ...
      init: function(mainElmId) {
        ...
        Avast.ABEK.Panel.getCurrentTabDetails(function(tabId, url) {
          this.msgs = Avast.ABEK.Panel.initMessaging('BE.Panel', tabId, url);
      ...
      handle_initPanel: function(msg) {
        this.ractive = new Ractive ({
          ...
          oninit: function(event) {
            this.msgs.sendMessage('viewInit');
          }.bind(this)
        });
