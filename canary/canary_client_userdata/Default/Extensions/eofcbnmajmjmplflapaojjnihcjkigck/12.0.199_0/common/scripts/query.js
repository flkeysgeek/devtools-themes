(function(_, AvastWRC, PROTO) {
  ...
  ************************************************************************************************************/
  ...
  AvastWRC.Query.SafeShopOffer = function(options) {
      ...
      this.request = new AvastWRC.gpb.All.SafeShopOffer.Request;
      this.response = new AvastWRC.gpb.All.SafeShopOffer.Response;
  ...
  AvastWRC.Query.SafeShopOffer.prototype = _.extend({},AvastWRC.Query.__MASTER__,{
     ...
     */
      ...
      //-- TODO - will be served by proxy server
          this.request.tag =  AvastWRC.bal.brandingType == undefined || AvastWRC.bal.brandingType == AvastWRC.BRANDING_TYPE_AVAST ? "avast" : "avg";
      ...
      //------
          ...
          this.request.status = AvastWRC.gpb.All.SafeShopOffer.Request.UserStatus[this.options.status];
  ...
  AvastWRC.Query.SafeShopPickedOffer = function(options) {
      if(!options.originalRequest || !options.offer) return false; // no page data

      this.options = _.extend({
          server : AvastWRC.Query.CONST.PICKED_OFFER_SERVER,
          method : "post",
      ...
      this.request  = new AvastWRC.gpb.All.SafeShopOffer.PickedOffer.Request;
      this.response = new AvastWRC.gpb.All.SafeShopOffer.PickedOffer.Response;

      this.init();
  };

  AvastWRC.Query.SafeShopPickedOffer.prototype = _.extend({},AvastWRC.Query.__MASTER__,{
    /**
     * build PROTO message
     */
      message : function() {
          var offer_fields = [
              ...
              "coupon_new_customers", "domain_label", "value", "created", "external_id", /*"time",*/ "views", "recommended",
          ];

          this.request.originalRequest = new AvastWRC.gpb.All.SafeShopOffer.Request;
          //-- TODO - will be served by proxy server
          this.request.originalRequest.tag = AvastWRC.bal.brandingType == undefined || AvastWRC.bal.brandingType == AvastWRC.BRANDING_TYPE_AVAST ? "avast" : "avg";
          ...
          if(this.options.offer.category && this.options.offer.category === "Voucher"){
              this.request.originalRequest.voucherSearch = true;              
          }     
          this.request.originalRequest.query = JSON.stringify(this.options.originalRequest.query);
          this.request.originalRequest.status = AvastWRC.gpb.All.SafeShopOffer.Request.UserStatus[this.options.originalRequest.status];

          this.request.originalRequest.identity = this.identity();

          this.request.offer = new AvastWRC.gpb.All.SafeShopOffer.Response.Offer;
